...
def get_validation_detail(upload_id: str, key: str, state: str | None = None):
    ...
    try:
        ...
        if key == "lookalike_parties":
            ...
            for p in pairs:
                rec = dict(p)
                rec["tl_name"] = _dominant_tl_for_lookalike_pair(p, state_to_tl)
                rec["pair_key"] = _lookalike_pair_key(p)
                tagged.append(rec)

            by_tl = {}
            for rec in tagged:
                by_tl[rec["tl_name"]] = by_tl.get(rec["tl_name"], 0) + 1
            result["by_tl_missing"] = by_tl

            # "vs yesterday" + repeat-offender flag, keyed on the
            # order-independent pair identity. Computed unconditionally
            # regardless of any active state filter — there's no per-pair
            # state anyway (each pair spans two parties across possibly
            # multiple states), so "state filter" doesn't apply here.
            prev_date, prev = database.get_previous_check_snapshot_with_date(db, "lookalike_parties", date.today().isoformat())
            if prev:
                result["prev_total_count"] = prev.get("total_count")
                result["prev_flagged_count"] = prev.get("flagged_count")
                result["prev_by_tl_missing"] = prev.get("by_tl_missing", {})

                prev_keys = set(prev.get("flagged_pair_keys", []))
                for rec in tagged:
                    rec["repeat"] = rec["pair_key"] in prev_keys
            else:
                for rec in tagged:
                    rec["repeat"] = False

            result["records"] = tagged
